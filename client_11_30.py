    ...
    def local_validate(self,model):
        ...
        with torch.no_grad():
            for images, labels in self.testDataloader:
                group_labels.clear()
                group_images.clear()
                tensor_group_labels = torch.tensor(())
                tensor_group_images = torch.tensor(())
                
                #print("label %s", images)
                #print("SIZE", labels.size(dim=0), images.size(dim=0) )
                
                #print ((labels == 1).nonzero(as_tuple=True)[0])
                #print(torch.where(labels == 1))
                
                i = 0
                x = torch.tensor(1)
                while(i < labels.size(dim=0)):
                    #why dont python allow i++
                    if(labels[i] == x):
                        w = images[i]
                        z = labels[i]
                        
                        #tensor_group_labels = torch.cat((tensor_group_labels, z), 0)
                        tensor_group_images = torch.cat((tensor_group_images, w), 2)
                        #print("IN LOOP", i, "IMAGE", a)
                        #group_images.append(images[i])
                        group_labels.append(labels[i].item())
                        #print("HERE ",i , "list: ", group_labels)
                        #print("i: ",i, "lables ", images[i].item())
                    i+=1
                #print("HERE", group_labels)
                #print("HERE", a)
                
                tensor_group_labels = torch.Tensor(group_labels)
                tensor_group_images = tensor_group_images
                
                print("-----------group 1-------------------")
                print(tensor_group_labels)
                print("-------------------------------------")
                print(tensor_group_images)
                print("-------------------------------------")
                #print(images)
                ...
                images, labels = tensor_group_images.to(self.device), tensor_group_labels.to(self.device)
                if images.shape[1]==1:
