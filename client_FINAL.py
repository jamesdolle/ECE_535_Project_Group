...
class LocalBase():
    def  __init__(self,args,train_dataset,test_dataset,client_id):
        ...
        #print(type(data_test_noise),"&&&&")
        data_test_noise=self.TERM(data_test_noise)
        dataSet_train = self.TERM(dataSet_train)
        ...
        #print(type(self.trainDataset),"=====")
        #print(type(self.testDataset),"#####")
    ...
    def TERM(self,noise_data):
        ...
        torch_transforms = type(noise_data[0][0])
        #print(type(noise_data[0][0]),"````")
        #print(type(noise_data[0][0][0]),"$$$$")
        #print(type(noise_data[0][0][0][0]),"((()))")
        #print(torch.max(noise_data))
        #print(noise_data[0][1][0][0][0])
        for c in range(len(noise_data)):
          #if type(noise_data[c])==torch_transforms:
             #transforms.functional.adjust_contrast(noise_data[c],1.6)
          if self.is1DList(noise_data[c]) == 1:
             ...
             for d in range(len(noise_data[c])):
               if type(noise_data[c][d])==torch_transforms:
                  tens = noise_data[c][d] 
                  transf = T.ToPILImage()
                  img = transf(tens)
                  T.functional.adjust_contrast(img,1.6)
                  T.functional.adjust_saturation(img,1.4)
